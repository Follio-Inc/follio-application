  ...
  const handleOAuth = async (provider: 'oauth_google' | 'oauth_github' | 'oauth_linkedin_oidc') => {
    ...
    try {
      await signUp.authenticateWithRedirect({
        ...
        redirectUrlComplete: '/onboarding/import',
      });
    } catch (err: unknown) {
      const clerkError = err as { errors?: Array<{ message: string }> };
      setError(clerkError.errors?.[0]?.message || 'OAuth sign-up failed. Please try again.');
    ...
    <Card className="w-full max-w-md rounded-2xl border shadow-lg">
      <CardHeader className="space-y-4 text-center">
        ...
        <div>
          ...
          <CardDescription className="mt-1">Start building your digital resume</CardDescription>
        </div>
      </CardHeader>
