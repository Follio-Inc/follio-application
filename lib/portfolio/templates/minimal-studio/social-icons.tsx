...
/**
 * Minimal Studio Template — Social Icons
 *
 * Inline SVGs for crisp rendering. Rendered as small, quiet marks that inherit
...
export function SocialIcon({ type, className }: SocialIconProps) {
  const Icon = ICON_MAP[getSocialIconType(type)] ?? ICON_MAP.link;
  return (
...
/** A row of social links. `text` variant shows labelled, underlined links. */
export function SocialLinksRow({ links, variant = 'icon' }: SocialLinksRowProps) {
  if (links.length === 0) return null;

  if (variant === 'text') {
    return (
      <div className="ms-social-row ms-social-row--text">
        {links.map((link, i) => (
          <a
            key={i}
            href={link.url}
            ...
            className="ms-social-text-link"
          >
            {link.label || prettifyType(link.type)}
          </a>
        ))}
      </div>
    );
