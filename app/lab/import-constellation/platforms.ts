 ...
 * Lab-only: not wired into onboarding.
 */
...
/** Expanded editor width relative to idle square (user-requested width %). */
export const EXPAND_WIDTH_RATIO = 2.2;

export function expandedWidth(size: number): number {
  return Math.round(size * EXPAND_WIDTH_RATIO);
}
...
function separationNeeded(
  ...
  const roomX =
    signX > 0
      ? canvasW - margin - ahw - a.cx
      : a.cx - (margin + ahw);
  const roomY =
    signY > 0
      ? canvasH - margin - ahh - a.cy
      : a.cy - (margin + ahh);
...
export function layoutConstellation(
  ...
  const tiles: SimTile[] = platforms.map((p) => {
    ...
    return {
      ...
      cx: (p.x / 100) * canvasW,
      cy: (p.y / 100) * canvasH,
      w: expanding ? expandedWidth(p.size) : p.size,
      h: p.size,
      fixed: expanding,
    };
  });
