...
export function ShareDialog({ profile }: ShareDialogProps) {
  const { user: clerkUser } = useUser();
  const [open, setOpen] = useState(false);

  // Visibility state
  const [resumeVisibility, setResumeVisibility] = useState<ContentVisibility>(
    (profile as unknown as Record<string, ContentVisibility>).resumeVisibility ?? 'UNLISTED'
  );
            ...
            {resumeVisibility === 'PRIVATE' ? (
              ...
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  className="w-full resize-none overflow-hidden rounded-md border bg-muted/30 px-3 py-2 text-xs leading-relaxed text-foreground outline-none transition-colors focus:ring-1 focus:ring-ring"
                  rows={1}
                />
